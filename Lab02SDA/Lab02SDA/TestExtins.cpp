...
void testIteratorAnterior() {
	Dictionar d;
	IteratorDictionar id = d.iterator(); //iterator pe container vid
	assert(id.valid2() == false);

	for (int i = 0; i < 100; i++) {  //adaug de 100 de ori valoarea 33
		d.adauga(33, 33);
	}
	IteratorDictionar id2 = d.iterator(); //daca iterez doar 33 poate sa-mi dea iteratorul
	assert(id2.valid2() == true);
	TElem el = id2.element2();
	assert(el.first == 33);
	id2.anterior();
	assert(id2.valid2() == false);

	id2.prim(); //resetam pe ultimul element
	assert(id2.valid2() == true);
	...
	IteratorDictionar id3 = d2.iterator();
	assert(id3.valid2() == true);
	for (int i = 0; i < 200; i++) {
		id3.anterior();
	}
	assert(id3.valid2() == false);
	id3.prim();
	assert(id3.valid2() == true);
	...
	IteratorDictionar id4 = d3.iterator();
	assert(id4.valid2() == true);
	int count = 0;
	while (id4.valid2()) { //fiecare element e multiplu de 4 si sunt in total 50 de elemente
		TElem e = id4.element2();
		assert(e.first % 4 == 0);
		id4.anterior();
		count++;
	}
